<?php
	...
	function listaChamados($conexao, $filtro) {
    	...
    	if (tipoUsuario() == 'C')
			...
			$sql = "select a.coddep, c.desdep, a.codatn,
					...
					from f114cab a
					left join f999cpl b on a.codate=b.codusu
					left join f061dep c on a.coddep=c.coddep
					where codcli='$codusu'";
		else
			$sql = "select a.coddep, c.desdep, a.codatn,
							CASE a.nivpri WHEN '1' THEN 'Baixo'
							...
							END as sitatn, DATE_FORMAT(a.datger, '%d-%m-%Y') datger, DATE_FORMAT(a.datprv, '%d-%m-%Y') datprv,
							DATE_FORMAT(a.datatu,'%d-%m-%Y') datatu, DATE_FORMAT(a.datfim, '%d-%m-%Y') datfim, b.nomcom from f114cab a
							left join f999cpl b on a.codcli=b.codusu
							left join f061dep c on a.coddep=c.coddep
							where (codate='$codusu' or codate=0)";
		if ($filtro != "")
			$sql = $sql ." and $filtro";
	...
	function buscaChamadoPorCodigo($conexao, $codatn){
		...
		$query = "select coddep,codatn, codcli, codate, codusu, nivpri,
	...
	function insereChamado($conexao, $codcli, $codusu, $natatn, $descli, $coddep) {
		$query = "insert into f114cab (codatn, coddep, codcli, codate, codusu, datger, sitatn, datatu, natatn, descli)
					values (null, '$coddep', '$codcli', '0', '$codusu', SYSDATE(), '1', SYSDATE(), '$natatn', '$descli')";
